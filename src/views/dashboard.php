...
<div class="page-header">
    ...
    <div style="display:flex;align-items:center;gap:12px">
        <div class="auto-ping-control">
            ...
            <span style="font-size:0.75rem;color:rgba(255,255,255,0.4)">Auto Ping: <strong
                    style="color:#4ade80"><?= htmlspecialchars((string) ($pingInterval ?? '30')) ?>s</strong></span>
        </div>
        <div class="auto-ping-control">
            ...
            <select id="refreshInterval" onchange="setRefresh(this.value)"
                style="background:transparent;border:none;color:#94a3b8;font-size:0.75rem;cursor:pointer;font-family:'Inter',sans-serif">
            ...
            <span id="countdown"
                style="font-size:0.7rem;color:rgba(255,255,255,0.3);font-family:'JetBrains Mono',monospace;min-width:24px"></span>
        </div>
...
<div class="card-dark">
    <div class="card-header-dark"
        ...
        <h2><i class="fas fa-list"></i>Status</h2>
        <div style="display:flex;align-items:center;gap:14px">
            <!-- Customer Filter -->
            <div style="display:flex;align-items:center;gap:6px">
                <i class="fas fa-filter" style="color:rgba(255,255,255,0.25);font-size:0.72rem"></i>
                <select id="customerFilter" onchange="filterByCustomer(this.value)"
                    style="background:#1e293b;border:1px solid rgba(255,255,255,0.1);color:#e2e8f0;padding:5px 10px;border-radius:6px;font-size:0.8rem;cursor:pointer;font-family:'Inter',sans-serif;min-width:160px">
            ...
            <span id="lastUpdate" style="font-size:0.72rem;color:rgba(255,255,255,0.3)">Cập nhật:
                <?= date('H:i:s') ?></span>
        </div>
    </div>
            ...
            <tbody id="linesBody">
                <?php if (!empty($lines)): ?>
                    <?php foreach ($lines as $i => $line): ?>
                        <tr id="line-row-<?= $line['id'] ?>" data-customer-id="<?= $line['customer_id'] ?? '' ?>"
                            data-status="<?= $line['status'] ?>">
                            <td class="muted row-num"><?= $i + 1 ?></td>
                            <td class="mono" style="font-size:0.78rem"><?= htmlspecialchars($line['store_code'] ?? '—') ?></td>
                            <td class="customer-col hide-mobile">
                                <?= htmlspecialchars($line['customer_name_rel'] ?? $line['customer_name'] ?? '—') ?>
                            </td>
                            <td class="name-cell"><?= htmlspecialchars($line['name']) ?></td>
                            <td class="muted hide-mobile"><small><?= htmlspecialchars($line['branch_address'] ?? '') ?></small>
                            </td>
                            <td class="hide-mobile"><span class="mono"
                                    style="font-size:0.78rem"><?= htmlspecialchars($line['provider'] ?? '—') ?></span></td>
                            ...
                            <td>
                                <span
                                    class="response-time <?= ($line['avg_response_time'] ?? 0) < 50 ? 'good' : (($line['avg_response_time'] ?? 0) < 200 ? 'medium' : 'bad') ?>"
                                    id="rt-<?= $line['id'] ?>">
                                    <?= $line['avg_response_time'] ? round($line['avg_response_time'], 1) . ' ms' : '—' ?>
                                </span>
                            </td>
                            <td class="muted hide-mobile" id="lc-<?= $line['id'] ?>">
                                <?= $line['last_check'] ? date('d/m H:i:s', strtotime($line['last_check'])) : 'Never' ?>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="10" class="empty-state"><i class="fas fa-satellite-dish"></i> Chưa có line nào. <a
                                href="?action=lines" style="color:#06b6d4">Thêm ngay</a></td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
